...
import com.green.mapper.ProcesoMapper;
...
public class ProcesoServiceImpl implements ProcesoService {
    ...
    /**
     * Repository injected by Spring that manages Proceso entities
     *
     */
    @Autowired
    ...
    */
    ...
    public List<Proceso> getProceso() throws Exception {
        ...
        try {
            list = procesoRepository.findAll();
        } catch (Exception e) {
            log.error("finding all Proceso failed", e);
    ...
    public void saveProceso(Proceso entity) throws Exception {
        ...
        try {
            ...
            validateProceso(entity);

//            if (getProceso(entity.getIdProceso()) != null) {
//                throw new ZMessManager(ZMessManager.ENTITY_WITHSAMEKEY);
//            }

            procesoRepository.save(entity);
            log.debug("save Proceso successful");
        } catch (Exception e) {
    ...
    public void deleteProceso(Proceso entity) throws Exception {
        ...
        List<Subproceso> subprocesos = null;

        try {
            capsulas = capsulaRepository.findByProperty("proceso.idProceso",
                    entity.getIdProceso());
            ...
            subprocesos = subprocesoRepository.findByProperty("proceso.idProceso",
                    entity.getIdProceso());

            if (Utilities.validationsList(subprocesos) == true) {
                throw new ZMessManager().new DeletingException("subprocesos");
            }
    ...
    public void updateProceso(Proceso entity) throws Exception {
        ...
        try {
            ...
            procesoRepository.update(entity);

            log.debug("update Proceso successful");
        } catch (Exception e) {
    ...
    public List<ProcesoDTO> getDataProceso() throws Exception {
        try {
            List<Proceso> proceso = procesoRepository.findAll();

            List<ProcesoDTO> procesoDTO = new ArrayList<ProcesoDTO>();

            for (Proceso procesoTmp : proceso) {
                ProcesoDTO procesoDTO2 = procesoMapper.procesoToProcesoDTO(procesoTmp);
                procesoDTO.add(procesoDTO2);
            }
    ...
    public Proceso getProceso(Long idProceso) throws Exception {
        ...
        try {
            if (procesoRepository.findById(idProceso).isPresent()) {
                entity = procesoRepository.findById(idProceso).get();
            }
        } catch (Exception e) {
            log.error("get Proceso failed", e);
            throw new ZMessManager().new FindingException("Proceso");
        } finally {
        }
    ...
    public List<Proceso> findPageProceso(String sortColumnName,
        ...
        throws Exception {
        List<Proceso> entity = null;

        try {
            entity = procesoRepository.findPage(sortColumnName, sortAscending,
                    startRow, maxResults);
        } catch (Exception e) {
            throw new ZMessManager().new FindingException("Proceso Count");
        } finally {
        }
    ...
    public Long findTotalNumberProceso() throws Exception {
        ...
        try {
            entity = procesoRepository.count();
        } catch (Exception e) {
            throw new ZMessManager().new FindingException("Proceso Count");
        } finally {
        }
    ...
    * de busqueda voy a hacer.., ejemplo: where nombre=william o where nombre<>william,
    ...
    public List<Proceso> findByCriteria(Object[] variables,
        ...
        try {
            list = procesoRepository.findByCriteria(where);
        } catch (Exception e) {
