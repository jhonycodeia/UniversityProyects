...
import com.green.mapper.NotificacionMapper;
...
public class NotificacionServiceImpl implements NotificacionService {
    ...
    */
    ...
    public void validateNotificacion(Notificacion notificacion)
        ...
        try {
            ...
            if (constraintViolations.size() > 0) {
                ...
                for (ConstraintViolation<Notificacion> constraintViolation : constraintViolations) {
    ...
    public List<Notificacion> getNotificacion() throws Exception {
        log.debug("finding all Notificacion instances");
        ...
        try {
            ...
            log.error("finding all Notificacion failed", e);
    ...
    public void saveNotificacion(Notificacion entity) throws Exception {
        log.debug("saving Notificacion instance");

        try {
            if (entity == null) {
                throw new ZMessManager().new NullEntityExcepcion("Notificacion");
            }

            validateNotificacion(entity);

//            if (getNotificacion(entity.getIdNotificacion()) != null) {
//                throw new ZMessManager(ZMessManager.ENTITY_WITHSAMEKEY);
//            }

            notificacionRepository.save(entity);
            log.debug("save Notificacion successful");
        } catch (Exception e) {
            log.error("save Notificacion failed", e);
            throw e;
        } finally {
        }
    }
    ...
    public void deleteNotificacion(Notificacion entity)
        ...
        if (entity.getIdNotificacion() == null) {
            throw new ZMessManager().new EmptyFieldException("idNotificacion");
        }

        try {
            notificacionRepository.deleteById(entity.getIdNotificacion());
            log.debug("delete Notificacion successful");
        } catch (Exception e) {
            log.error("delete Notificacion failed", e);
            throw e;
        } finally {
        }
    }
    ...
    public void updateNotificacion(Notificacion entity)
        throws Exception {
        log.debug("updating Notificacion instance");
        ...
        try {
            ...
            notificacionRepository.update(entity);

            log.debug("update Notificacion successful");
        } catch (Exception e) {
            log.error("update Notificacion failed", e);
            throw e;
        } finally {
        }
    }
    ...
    public List<NotificacionDTO> getDataNotificacion()
        ...
        try {
            ...
            for (Notificacion notificacionTmp : notificacion) {
                NotificacionDTO notificacionDTO2 = notificacionMapper.notificacionToNotificacionDTO(notificacionTmp);
                notificacionDTO.add(notificacionDTO2);
            }
    ...
    public Notificacion getNotificacion(Long idNotificacion)
        throws Exception {
        log.debug("getting Notificacion instance");

        Notificacion entity = null;

        try {
            if (notificacionRepository.findById(idNotificacion).isPresent()) {
                entity = notificacionRepository.findById(idNotificacion).get();
            }
        } catch (Exception e) {
            log.error("get Notificacion failed", e);
            throw new ZMessManager().new FindingException("Notificacion");
        } finally {
        }
    ...
    public List<Notificacion> findPageNotificacion(String sortColumnName,
        ...
        throws Exception {
        List<Notificacion> entity = null;

        try {
            entity = notificacionRepository.findPage(sortColumnName,
                    sortAscending, startRow, maxResults);
        } catch (Exception e) {
            throw new ZMessManager().new FindingException("Notificacion Count");
        } finally {
        }
    ...
    public Long findTotalNumberNotificacion() throws Exception {
        ...
        try {
            entity = notificacionRepository.count();
        } catch (Exception e) {
            throw new ZMessManager().new FindingException("Notificacion Count");
        } finally {
        }
    ...
    * de busqueda voy a hacer.., ejemplo: where nombre=william o where nombre<>william,
    ...
    public List<Notificacion> findByCriteria(Object[] variables,
        ...
        try {
            list = notificacionRepository.findByCriteria(where);
        } catch (Exception e) {
