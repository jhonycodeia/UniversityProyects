...
import com.green.mapper.DocumentoMapper;
...
public class DocumentoServiceImpl implements DocumentoService {
	...
	public void validateDocumento(Documento documento) throws Exception {
		try {
			Set<ConstraintViolation<Documento>> constraintViolations = validator.validate(documento);

			if (constraintViolations.size() > 0) {
				StringBuilder strMessage = new StringBuilder();

				for (ConstraintViolation<Documento> constraintViolation : constraintViolations) {
	...
	public List<Documento> getDocumento() throws Exception {
		...
		try {
			list = documentoRepository.findAll();
		} catch (Exception e) {
			log.error("finding all Documento failed", e);
			throw new ZMessManager().new GettingException(ZMessManager.ALL + "Documento");
		} finally {
		}
	...
	public void saveDocumento(Documento entity) throws Exception {
		...
		try {
			...
			validateDocumento(entity);

//            if (getDocumento(entity.getIdDocumento()) != null) {
//                throw new ZMessManager(ZMessManager.ENTITY_WITHSAMEKEY);
//            }

			documentoRepository.save(entity);
			log.debug("save Documento successful");
		} catch (Exception e) {
			log.error("save Documento failed", e);
			throw e;
		} finally {
		}
	}
	...
	public void deleteDocumento(Documento entity) throws Exception {
		...
		if (entity.getIdDocumento() == null) {
			throw new ZMessManager().new EmptyFieldException("idDocumento");
		}

		try {
			s3Services.deleteObject(Constantes.BUCKET_NAME, entity.getRuta());
			documentoRepository.deleteById(entity.getIdDocumento());
			log.debug("delete Documento successful");
		} catch (Exception e) {
			log.error("delete Documento failed", e);
			throw e;
		} finally {
		}
	}
	...
	public void updateDocumento(Documento entity) throws Exception {
		log.debug("updating Documento instance");
		...
		try {
			...
			documentoRepository.update(entity);

			log.debug("update Documento successful");
		} catch (Exception e) {
			log.error("update Documento failed", e);
			throw e;
		} finally {
		}
	}
	...
	public List<DocumentoDTO> getDataDocumento() throws Exception {
		try {
			...
			for (Documento documentoTmp : documento) {
				DocumentoDTO documentoDTO2 = documentoMapper.documentoToDocumentoDTO(documentoTmp);
	...
	public Documento getDocumento(Long idDocumento) throws Exception {
		log.debug("getting Documento instance");

		Documento entity = null;

		try {
			if (documentoRepository.findById(idDocumento).isPresent()) {
				entity = documentoRepository.findById(idDocumento).get();
			}
		} catch (Exception e) {
			log.error("get Documento failed", e);
	...
	@Transactional(readOnly = true)
	public List<Documento> findPageDocumento(String sortColumnName, boolean sortAscending, int startRow, int maxResults)
			throws Exception {
		List<Documento> entity = null;

		try {
			entity = documentoRepository.findPage(sortColumnName, sortAscending, startRow, maxResults);
		} catch (Exception e) {
	...
	public Long findTotalNumberDocumento() throws Exception {
		...
		try {
			entity = documentoRepository.count();
		} catch (Exception e) {
			throw new ZMessManager().new FindingException("Documento Count");
		} finally {
		}
	 ...
	 */
	@Transactional(readOnly = true)
	public List<Documento> findByCriteria(Object[] variables, Object[] variablesBetween, Object[] variablesBetweenDates)
			throws Exception {
		List<Documento> list = new ArrayList<Documento>();
		...
		try {
			list = documentoRepository.findByCriteria(where);
		} catch (Exception e) {
	...
	public InputStream generarFiles(Long idDocumento) throws Exception {
		try {
			...
			if (documento == null) {
				throw new Exception("El documento no se logro encontrar");
			}

			return s3Services.download(Constantes.BUCKET_NAME, documento.getRuta());
